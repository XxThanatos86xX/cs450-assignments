...
void Display( )
{
	...
	switch (farOrClose) {
		case 0:
			gluLookAt( 0, 150, 150,		0, 0, 0,    0, 1, 0 );		// close view
			break;
		case 1:
			gluLookAt( 0, 300, 300,		0, 0, 0,    0, 1, 0 );		// far view
			break;
	}
	...
	glPushMatrix();
		...
		radius = 24.065;		// orbital radius
		orbitPeriod = .24;
		glDisable(GL_LIGHTING);
		glDisable(GL_TEXTURE_2D);
		glColor3f(0, 0, 1);
		glCallList(mercuryOrbit);
		glEnable(GL_LIGHTING);

		glEnable(GL_LIGHT0);
		glRotatef(720*(pow(radius, 3/2) / orbitPeriod)*Time, 0, 1, 0);
		glTranslatef(radius + sunRadius, 0, 0);
...
InitGraphics( )
{
	...
	file = (char*) "./Textures/mercury.bmp";
...
InitLists( )
{
	...
	mercuryOrbit = glGenLists( 1 );
	glNewList( mercuryOrbit, GL_COMPILE);
		glPushMatrix();
		glBegin(GL_LINE_STRIP);
			for(int i = 0; i < NUMSEGS; i++) {
				glVertex3f( (24.065 + sunRadius)*cos(ang), 0, (24.065 + sunRadius)*sin(ang) );
				ang += dang;
			}
		glEnd();
	glPopMatrix();
	...
	venusOrbit = glGenLists( 1 );
	glNewList( venusOrbit, GL_COMPILE);
		glPushMatrix();
		glBegin(GL_LINE_STRIP);
			for(int i = 0; i < NUMSEGS; i++) {
				glVertex3f( (53.4 + sunRadius)*cos(ang), 0, (53.4 + sunRadius)*sin(ang) );
				ang += dang;
			}
		glEnd();
	glPopMatrix();
	...
	glNewList( earthOrbit, GL_COMPILE);
		...
		glBegin(GL_LINE_STRIP);
			for(int i = 0; i < NUMSEGS; i++) {
				glVertex3f( (73.35 + sunRadius)*cos(ang), 0, (73.35 + sunRadius)*sin(ang) );
				ang += dang;
			}
		glEnd();
	glPopMatrix();
	...
	glNewList( marsOrbit, GL_COMPILE);
		...
		glBegin(GL_LINE_STRIP);
			for(int i = 0; i < NUMSEGS; i++) {
				glVertex3f( (112.25 + sunRadius)*cos(ang), 0, (112.25 + sunRadius)*sin(ang) );
				ang += dang;
			}
		glEnd();
	glPopMatrix();
	...
	jupiterOrbit = glGenLists( 1 );
	glNewList( jupiterOrbit, GL_COMPILE);
		glPushMatrix();
		glBegin(GL_LINE_STRIP);
			for(int i = 0; i < NUMSEGS; i++) {
				glVertex3f( (186.275 + sunRadius)*cos(ang), 0, (186.275 + sunRadius)*sin(ang) );
				ang += dang;
			}
		glEnd();
	glPopMatrix();
	...
	saturnOrbit = glGenLists( 1 );
	glNewList( saturnOrbit, GL_COMPILE);
		glPushMatrix();
		glBegin(GL_LINE_STRIP);
			for(int i = 0; i < NUMSEGS; i++) {
				glVertex3f( (365 + sunRadius)*cos(ang), 0, (365 + sunRadius)*sin(ang) );
				ang += dang;
			}
		glEnd();
	glPopMatrix();
	...
	uranusOrbit = glGenLists( 1 );
	glNewList( uranusOrbit, GL_COMPILE);
		glPushMatrix();
		glBegin(GL_LINE_STRIP);
			for(int i = 0; i < NUMSEGS; i++) {
				glVertex3f( (490 + sunRadius)*cos(ang), 0, (490 + sunRadius)*sin(ang) );
				ang += dang;
			}
		glEnd();
	glPopMatrix();
	...
	neptuneOrbit = glGenLists( 1 );
	glNewList( neptuneOrbit, GL_COMPILE);
		glPushMatrix();
		glBegin(GL_LINE_STRIP);
			for(int i = 0; i < NUMSEGS; i++) {
				glVertex3f( (558.75 + sunRadius)*cos(ang), 0, (558.75 + sunRadius)*sin(ang) );
				ang += dang;
			}
		glEnd();
	glPopMatrix();
    ...
    glNewList(sunDL, GL_COMPILE);
        glBindTexture( GL_TEXTURE_2D, sunTex );
		glPushMatrix();
			glScalef(sunRadius, sunRadius, sunRadius);	// reduced 1/4th
			glEnable(GL_TEXTURE_2D);
